...
cv::Mat_<double> GetMeanShape(const vector<cv::Mat_<double> >& shapes,
                          const vector<BoundingBox>& bounding_box){
    cv::Mat_<double> result = cv::Mat::zeros(shapes[0].rows,2,CV_64FC1);
...
void GetShapeResidual(const vector<cv::Mat_<double> >& ground_truth_shapes,
                      const vector<cv::Mat_<double> >& current_shapes,
                      const vector<BoundingBox>& bounding_boxs,
                      const cv::Mat_<double>& mean_shape,
                      vector<cv::Mat_<double> >& shape_residuals){
...
void SimilarityTransform(const cv::Mat_<double>& shape1, const cv::Mat_<double>& shape2, 
    ...
    // calculate covariance cv::Matrix
...
cv::Mat_<double> LoadGroundTruthShape(string& filename){
    cv::Mat_<double> shape(global_params.landmark_num,2);
    ifstream fin;
    string temp;
...
void LoadData(string filepath,
    ...
    while(getline(fin,name)){
        ...
        cv::Mat_<double> ground_truth_shape = LoadGroundTruthShape(name);
...
void LoadCofwTrainData(vector<cv::Mat_<uchar> >& images,
    ...
    for(int i = 0;i < img_num;i++){
        ...
        cv::Mat_<uchar> temp = cv::imread(image_name,0);
    ...
    for(int i = 0;i < img_num;i++){
        cv::Mat_<double> temp(global_params.landmark_num,2);
...
void LoadCofwTestData(vector<cv::Mat_<uchar> >& images,
                      vector<cv::Mat_<double> >& ground_truth_shapes,
                      vector<BoundingBox>& bounding_boxs){
    int img_num = 507;
    cout<<"Read images..."<<endl;
