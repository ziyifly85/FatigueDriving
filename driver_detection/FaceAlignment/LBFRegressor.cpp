...
struct feature_node ** LBFRegressor::DeriveBinaryFeat(
	...
	const vector<cv::Mat_<uchar> >& images,
...
void LBFRegressor::GlobalRegression(struct feature_node **binfeatures,
	const vector<cv::Mat_<double> >& shapes_residual,
	vector<cv::Mat_<double> >& current_shapes,
	const vector<BoundingBox> & bounding_boxs,
...
void LBFRegressor::Train(const vector<cv::Mat_<uchar> >& images,
	const vector<cv::Mat_<double> >& ground_truth_shapes,
	const vector<BoundingBox> & bounding_boxs) {

	// data augmentation and multiple initialization
	vector<cv::Mat_<uchar> > augmented_images;
	vector<BoundingBox> augmented_bounding_boxs;
	vector<cv::Mat_<double> > augmented_ground_truth_shapes;
	vector<cv::Mat_<double> > current_shapes;
...
vector<cv::Mat_<double> > LBFRegressor::Predict(const vector<cv::Mat_<uchar> >& images,
	const vector<BoundingBox>& bounding_boxs,
	const vector<cv::Mat_<double> >& ground_truth_shapes,
	int initial_num) {
