...
inline double calculate_var(const cv::Mat_<double>& v1){
    double mean_1 = mean(v1)[0];
    double mean_2 = mean(v1.mul(v1))[0];
    return mean_2 - mean_1*mean_1;
...
void Tree::Train(const vector<cv::Mat_<uchar> >& images,
                 const vector<cv::Mat_<double> >& ground_truth_shapes,
                 const vector<cv::Mat_<double> >& current_shapes,
                 const vector<BoundingBox> & bounding_boxs,
                 const cv::Mat_<double>& mean_shape,
                 const vector<cv::Mat_<double> >& regression_targets,
    ...
    int num_nodes_iter;
    int num_split;
	cv::Mat_<double> shapes_residual((int)index.size(),2);
...
void Tree::Splitnode(const vector<cv::Mat_<uchar> >& images,
                     const vector<cv::Mat_<double> >& ground_truth_shapes,
                     const vector<cv::Mat_<double> >& current_shapes,
	...
	cv::Mat_<double> candidate_pixel_locations(max_numfeats_,4);
	...
	cv::Mat_<int> densities(max_numfeats_,(int)ind_samples.size());
    for (int i = 0;i < ind_samples.size();i++){
		cv::Mat_<double> rotation;
        double scale;
		cv::Mat_<double> temp = ProjectShape(current_shapes[ind_samples[i]],bounding_box[ind_samples[i]]);
