...
FaceDetection::FaceDetection(const std::vector<std::string> model_file, const std::vector<std::string> train_model)
{
#ifdef CPU_ONLY
	...
	for (int i = 0; i < model_file.size(); ++i)
	{
		...
		net->CopyTrainedLayersFrom(train_model[i]);
		...
		nets_.push_back(net);
		input_size_.push_back(input_size);
		if (i == 0)
...
void FaceDetection::detection(const cv::Mat& img, std::vector<cv::Rect>& rectangles)
{
	Preprocess(img);
	P_Net();
	local_NMS();
...
void FaceDetection::detection(const cv::Mat& img, std::vector<cv::Rect>& rectangles, std::vector<float>& confidence)
{
...
void FaceDetection::detection(const cv::Mat& img, std::vector<cv::Rect>& rectangles, std::vector<float>& confidence, std::vector<std::vector<cv::Point>>& alignment)
{
...
void FaceDetection::Preprocess(const cv::Mat &img)
{
...
void FaceDetection::R_Net()
{
	detect_net(1);
}

void FaceDetection::O_Net()
{
	detect_net(2);
}

void FaceDetection::detect_net(int i)
{
	float thresh = this->threshold_[i];
	...
	for (int j = 0;j < bounding_box_.size(); ++j) {
		...
		if (img.rows == 0 || img.cols == 0)
			continue;
		if (img.size() != input_size_[i]) 
			cv::resize(img, img, input_size_[i]);
...
*/
void FaceDetection::Predict(const cv::Mat& img, int i)
{
...
*/
void FaceDetection::Predict(const std::vector<cv::Mat> imgs, int i)
{
	std::shared_ptr<Net<float>> net = nets_[i];

	Blob<float>* input_layer = net->input_blobs()[0];
	input_layer->Reshape(imgs.size(), num_channels_,
		input_size_[i].height, input_size_[i].width);
...
void FaceDetection::WrapInputLayer(const cv::Mat& img, std::vector<cv::Mat> *input_channels, int i)
{
...
*/
void FaceDetection::WrapInputLayer(const vector<cv::Mat> imgs, std::vector<cv::Mat> *input_channels, int i)
{
	Blob<float> *input_layer = nets_[i]->input_blobs()[0];
...
inline float FaceDetection::IoU(cv::Rect rect1, cv::Rect rect2)
{
	...
	unions = rect1.width * rect1.height + rect2.width * rect2.height - intersection;	//两个区域的并集
	return float(intersection) / unions;
}


float FaceDetection::IoM(cv::Rect rect1, cv::Rect rect2)
{
	int x_overlap, y_overlap, intersection, min_area;
	x_overlap = std::max(0, std::min((rect1.x + rect1.width), (rect2.x + rect2.width)) - std::max(rect1.x, rect2.x));
	y_overlap = std::max(0, std::min((rect1.y + rect1.height), (rect2.y + rect2.height)) - std::max(rect1.y, rect2.y));
	intersection = x_overlap * y_overlap;
	min_area = std::min((rect1.width * rect1.height), (rect2.width * rect2.height));	//两个区域较小值
	return float(intersection) / min_area;
}

void FaceDetection::resize_img()
{
	cv::Mat img = img_;
	int height = img.rows;
	int width = img.cols;
...
void FaceDetection::GenerateBoxs(cv::Mat img)
{
	int stride = 2;
	int cellSize = input_size_[0].width;
...
void FaceDetection::BoxRegress(std::vector<cv::Rect>& bounding_box, std::vector<cv::Rect> regression_box)
{
...
void FaceDetection::Padding(std::vector<cv::Rect>& bounding_box, int img_w, int img_h)
{
	for (int i = 0; i<bounding_box.size(); i++)
	{
...
cv::Mat FaceDetection::crop(cv::Mat img, cv::Rect& rect)
{
...
void FaceDetection::img_show(cv::Mat img, std::string name)
{
	cv::Mat img_show;
	img.copyTo(img_show);
...
void FaceDetection::img_show_T(cv::Mat img, std::string name)
{
	cv::Mat img_show;
	img.copyTo(img_show);
